...
class LearnableMaskLayer(nn.Module):
    ...
    def _icnn_mask(self, x, labels, epoch):
        ...
        mask_zeros = torch.zeros(mask[:, 0].shape, device=x.device)
        # mask_zeros += self.mask[:, 5] # mask first class channels
        # mask_zeros += self.mask[:, 1]  # mask first class channels
        mask_zeros += torch.sum(mask[:], dim=-1)
        mask_zeros -= mask[:, 0]  # mask first class channels
        mask_zeros -= mask[:, 5]  # mask first class channels
