...
class VGG(nn.Module):
    ...
    def forward(self, x, labels, epoch):
        ...
        # label_ones[:, 0 * label_filter_range:1 * label_filter_range, :, :] = 0.0
        # label_ones[:, 4 * label_filter_range:5 * label_filter_range, :, :] = 0
        # label_ones[:10*label_filter_range:,:,:] = 0

        # random filters drop
        random_filter_indices = torch.from_numpy(np.random.randint(low=0, high=x.shape[1], size=2*label_filter_range))
