...
parser.add_argument('--maxiter', default=75, type=int, help='The maximum number of iteration in the DE algorithm.')
...
def main():
    ...
    test_set = torchvision.datasets.CIFAR10(root=data_dir, train=False, download=True, transform=tranfrom_test)
    ...
    PATH = '../tb_dir/cifar_exp/test_256bs_150epoch_layer3_open'
    # PATH = '../tb_dir/cifar_exp/baseline'
    model_path = os.path.join(PATH, 'saved_model.pt')
    model = resnet_cifar(depth=20, num_classes=10, mask_value=0)
    model = model.cuda()
    ...
    results = attack_all(model, testloader, pixels=args.pixels, targeted=args.targeted, maxiter=args.maxiter,
